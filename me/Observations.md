# 2026-05-04: Evaluating the Effectiveness of the First Schedule
![[IMG_0143.jpg]]
*A house in Stone Ridge, NY, sits on a hill on it's own estate. The Mohonk Mountains and Shawangunks are barely visible. (2021)*
#### Benefits
Here are some takeaways from the initial schedule. It's available in this folder for archival purposes.
- **Consistency:** For the first time ever, I was able to stick to the schedule on a consistent basis. The schedule was put into practice in late February/early March, and I was able to stick to it for about 3-4 weeks in total. Almost every single day looked the same in terms of it's structure, and I managed to follow the schedule somewhat closely. I managed to wake up and go to sleep at somewhat regular intervals, however, sleep was somewhat disrupted.
- **Productivity:** Unprecedented levels of productivity were achieved during this period of time. Motivation levels remained highest between 12:00 and 16:00, with it fizzling out around 18:00. This was on $18$ $mg$ Concerta medication. In this period of time, I remained entirely focused on digital artwork and Blender.  
- **Mood:** I achieved a great amount of self-satisfaction during this time period. Although I did not necessarily look forward to the next day, I felt as if I had some kind of purpose for the first time, doing work that I felt was meaningful to myself. Getting such work done, on a consistent basis, was foreign. 
#### Shortcomings
- **Task-switching:** Eating remained difficult and somewhat inconsistent throughout this entire time period. Although I became more consistent and disciplined over time, it overall remained a difficult, conscious task that I had to go out of my way to do.
- **Sustainability:** This period of motivation was only sustainable for about a month. Subsequent of returning home, I have been unable to achieve this level of focus or replicate any of these behaviors once. There are several factors to this, but it has been over a month at home, and I still have not been able to replicate my previous level of discipline.
#### Conclusion
The first schedule, combined with medication, shows promise and illustrates what the self is capable of. The work and discipline from the "golden period" of the schedule remains strong proof of the existence of purpose and the benefits of a sort of delayed gratification. It has remained deeply de-motivating that I have struggled to replicate this discipline, however, these are extremely high standards for one to hold himself to.

Established is that there exists a desire to achieve or live up to this standard which I have now set for myself. Such a standard can be achieved iff there is something to motivate it. The ontogeny of so-called "motivation", however, is unclear, and inconsistent. Establishing goals with a tautological existence, which do not require a kind of motivation, would appear to be the logical next step.
#### Next Steps
Some of the ideas for the next schedule framework:
* Modularity in the framework, with some immutable aspects that add consistency.
* Greater precision, as the previous schedule had a *de facto* wake-up and curfew time. These times must be regulated as it's proven to be extremely difficult to keep with this in more recent times.
* More outdoors/physical activity: Not much justification is necessary for this, it's just healthy.
* Reintroduce the journaling, maybe make it's home here in Obsidian instead so that it's agnostic across both the Lenowo and the PC.

The new schedule has been designated. Come back in about a week or so and add some notes on it here.

# 2026-06-16
It has been more than just one week since the previous revision, however, much has been learned about the schedule in this time period.

Some of my thoughts and feelings are available in my journal entry for 2026-06-15, however, I'll provide some thoughts in this space anyway.
- Some components of the schedule seem to have persisted over the entire month, I've generally practiced a good morning and evening routine.
- Journaling has continued over not just the past month, but overall for this entire year of 2026 up until now.
- Progress on the SIII has been motivated, albeit at a much slower rate than in February/March of 2026. The modeling process has been completed and cleanup of the geometry is now in order.
- Adhering rigidly to the schedule has been extremely difficult and cannot be sustained for more than a few days at a time.

I've made some revisions to the schedule which is noted by the new name "Schedule v2.1". The most notable change is an increased tolerance for error as wake-up is now at 8:15 rather than 7:00. As the latter time window was too early, I tended to wake up at 7:00, feel sleepy, go back to bed, and then start the day much later, as late as 10:30 or 11:00. 

With the new schedule, I can sleep as late as 23:00, wake up at 8:15, and still have approximately 9 hours of sleep. Insofar as I can follow up in taking my medication on time, the effectiveness of the medication should persist in the new schedule.
# 2026-06-18: Hobbies and Activities
The new schedule has worked okay so far. No hiccups, and I'm generally getting along with the schedule quite well, but I have noticed that I usually conclude my activities a little bit earlier than 18:30. The resulting "dead space" has actually turned out to be a much larger problem than I expected it to be. As there is nothing planned for that time, I get bored and don't really know what to do. 

The policy against this is to produce a list of potential activities, which is a supposed obvious solution. I'll produce an incomplete list of things to do here. That way, whenever I get to a point where I feel bored or like there's nothing to do, I can just fall back to this. If I have to, I'll flip a coin or roll some dice to decide, I don't know.
1. Relearning and retraining on first-order logic
2. RedHat System Administration certification
3. Maintenance on Linux systems that may not have been updated in a while
4. Revisiting the Windows partition to ensure it's integrity and that it is up-to-date
5. Perspective pencil drawings
6. Character pencil drawings
7. List another keyboard or item on eBay
8. Vacuum, clean/mop the room
9. Travel to a new MNR station and railfan there - for days on which work is finished quite early
10. Archival of media on the KBI Wiki or archive.org
11. Cleanup and integrity check of the filesystem on all devices
12. Synchronizing files between all devices
13. Cleaning peripherals - keyboard, mouse, headphones
14. Check for new listings on eBay
15. Go out for a walk
16. Downloading music + transfer to phone
17. 